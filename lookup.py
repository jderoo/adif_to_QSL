...
class QRZlookup:
    ...
    def lookup(self, call: str) -> tuple:
        ...
        if self.session:
            ...
            grid, name, nickname, email, error_text = self.parse_lookup(query_result)
        return grid, name, nickname, email, error_text
    ...
    def parse_lookup(self, query_result):
        ...
        if query_result.status_code == 200:
            ...
            if root.find("callsign"):
                ...
                if root.callsign.find("nickname"):
                    nickname = root.callsign.nickname.text
                if root.callsign.find("email"):
                    email = root.callsign.email.text
        logging.info(
            "QRZlookup-parse_lookup: %s %s %s %s",
            grid,
            name,
            nickname,
            email,
